...
@interface RNSketchImageEditorData ()
...
@implementation RNSketchImageEditorData
{
    CGRect _dirty;
    UIBezierPath *_path;
}
...
- (instancetype)initWithId:(int) pathId strokeColor:(UIColor*) strokeColor strokeWidth:(int) strokeWidth {
    ...
    if (self) {
        ...
        _isTranslucent = CGColorGetComponents(strokeColor.CGColor)[3] != 1.0 &&
            ![RNSketchImageEditorUtility isSameColor:strokeColor color:[UIColor clearColor]];
...
- (CGRect)addPoint:(CGPoint) point {
    ...
    if (_isTranslucent) {
        if (pointsCount >= 3) {
            [RNSketchImageEditorUtility addPointToPath: _path
                            toPoint: point
            ...
            [RNSketchImageEditorUtility addPointToPath: _path toPoint: point tertiaryPoint: point previousPoint: point];
        }
        ...
        if (pointsCount >= 3) {
            ...
            CGPoint c = point;
            CGPoint prevMid = [RNSketchImageEditorUtility midPoint:a p2:b];
            CGPoint currentMid = [RNSketchImageEditorUtility midPoint:b p2:c];
            ...
            CGPoint b = point;
            CGPoint mid = [RNSketchImageEditorUtility midPoint:a p2:b];
...
- (void)drawInContext:(CGContextRef)context pointIndex:(NSUInteger)pointIndex {
    ...
    BOOL isErase = [RNSketchImageEditorUtility isSameColor:_strokeColor color:[UIColor clearColor]];
