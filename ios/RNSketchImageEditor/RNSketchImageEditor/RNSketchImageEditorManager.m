...
@implementation RNSketchImageEditorManager
...
#pragma mark - Props
RCT_CUSTOM_VIEW_PROPERTY(shapeConfiguration, NSDictionary, RNSketchImageEditor)
{
    RNSketchImageEditor *currentView = !view ? defaultView : view;
...
RCT_CUSTOM_VIEW_PROPERTY(localSourceImage, NSDictionary, RNSketchImageEditor)
{
    RNSketchImageEditor *currentView = !view ? defaultView : view;
    NSDictionary *dict = [RCTConvert NSDictionary:json];
    dispatch_async(dispatch_get_main_queue(), ^{
...
RCT_CUSTOM_VIEW_PROPERTY(text, NSArray, RNSketchImageEditor)
{
    RNSketchImageEditor *currentView = !view ? defaultView : view;
...
- (UIView *)view
{
    return [[RNSketchImageEditor alloc] initWithEventDispatcher: self.bridge.eventDispatcher];
}
...
{
    [self runCanvas:reactTag block:^(RNSketchImageEditor *canvas) {
...
- (void)runCanvas:(nonnull NSNumber *)reactTag block:(void (^)(RNSketchImageEditor *canvas))block {
    [self.bridge.uiManager addUIBlock:
     ^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, RNSketchImageEditor *> *viewRegistry){

         RNSketchImageEditor *view = viewRegistry[reactTag];
         if (!view || ![view isKindOfClass:[RNSketchImageEditor class]]) {
             RCTLogError(@"Cannot find RNSketchImageEditor with tag #%@", reactTag);
             return;
         }
