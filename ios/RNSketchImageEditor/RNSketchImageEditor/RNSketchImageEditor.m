#import "RNSketchImageEditorManager.h"
#import "RNSketchImageEditor.h"
#import "RNSketchImageEditorData.h"
...
@implementation RNSketchImageEditor
{
    RCTEventDispatcher *_eventDispatcher;
    NSMutableArray *_paths;
    RNSketchImageEditorData *_currentPath;
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_needsFullRedraw) {
        ...
        for (RNSketchImageEditorData *path in _paths) {
...
- (void)newPath:(int) pathId strokeColor:(UIColor*) strokeColor strokeWidth:(int) strokeWidth {
    ...
    _currentPath = [[RNSketchImageEditorData alloc]
                    initWithId: pathId
...
- (void) addPath:(int) pathId strokeColor:(UIColor*) strokeColor strokeWidth:(int) strokeWidth points:(NSArray*) points {
    ...
    for(int i=0; i<_paths.count; i++) {
        if (((RNSketchImageEditorData*)_paths[i]).pathId == pathId) {
            exist = true;
            break;
        }
    }
    ...
    if (!exist) {
        RNSketchImageEditorData *data = [[RNSketchImageEditorData alloc] initWithId: pathId
                                                  strokeColor: strokeColor
                                                  strokeWidth: strokeWidth
                                                       points: points];
...
- (UIImage*)createImageWithTransparentBackground: (BOOL) transparent includeImage:(BOOL)includeImage includeText:(BOOL)includeText cropToImageSize:(BOOL)cropToImageSize {
    if (_backgroundImage && cropToImageSize) {
        ...
        CGRect targetRect = [RNSketchImageEditorUtility fillImageWithSize:self.bounds.size toSize:rect.size contentMode:@"AspectFill"];
        CGFloat scaleFactor = [RNSketchImageEditorUtility getScaleDifference:self.bounds.size toSize:rect.size contentMode:@"AspectFill"];
        if (includeImage) {
        ...
        if (_backgroundImage && includeImage) {
            CGRect targetRect = [RNSketchImageEditorUtility fillImageWithSize:_backgroundImage.size toSize:rect.size contentMode:_backgroundImageContentMode];
...
- (UIImage *)scaleImage:(UIImage *)originalImage toSize:(CGSize)size contentMode: (NSString*)mode
{
    ...
    CGRect targetRect = [RNSketchImageEditorUtility fillImageWithSize:originalImage.size toSize:size contentMode:mode];
